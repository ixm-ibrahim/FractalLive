...
void main()
{
	...
	FragColor = vec4(Julia(), 1.0);FragPosWorld;bailoutTexture;useBailoutTexture;bailoutTextureScale;
}

vec2 JuliaLoop(vec2 c, inout int iter, inout vec2 trap, out vec4 domainZ, out ivec2 domainIter, inout float distanceEstimation, inout vec4 stripesAddend, float riemannAdjustment, out vec3 texCoords);
...
vec3 Julia()
{
    ...
    z = JuliaLoop(c, iter, trap, domainZ, domainIter, distanceEstimation, stripes, riemannAdjustment, texCoords);
